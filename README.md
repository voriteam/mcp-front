...
For Google identity providers, optional account revocation cuts off access when a user is suspended or deleted: at token refresh MCP Front re-verifies the user against the provider and, on rejection, denies the refresh and purges their stored service tokens and sessions. See `auth.workspaceRevocation` in the [configuration reference](docs-site/src/content/docs/configuration.md).
