...
// It fans stdout out to the OTel slog bridge, which is a no-op until a log
// provider is registered via go.opentelemetry.io/otel/log/global.
func updateHandler() {
	...
	otelBridge := otelslog.NewHandler("mcp-front")
...
// Enabled gates on the configured level rather than asking the handlers,
// because the OTel bridge answers yes at every level and would otherwise ship
// debug records that LOG_LEVEL excludes from stdout.
